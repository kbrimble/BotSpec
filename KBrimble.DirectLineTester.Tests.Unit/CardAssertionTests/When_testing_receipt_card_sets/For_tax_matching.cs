...
namespace KBrimble.DirectLineTester.Tests.Unit.CardAssertionTests.When_testing_receipt_card_sets
{
    ...
    public class For_a_tax_matching
    {
        ...
        [TestCase("")]
        [TestCase("symbols ([*])?")]
        public void HasTaxMatching_should_pass_if_regex_exactly_matches_message_Tax_of_one_card(string cardTaxAndRegex)
        {
        ...
        public void HasTaxMatching_should_pass_if_regex_exactly_matches_Tax_of_at_least_1_card_regardless_of_case(string cardTax, string regex)
        {
        ...
        [TestCase("^[j-z ]*$")]
        [TestCase("s{12}")]
        public void HasTaxMatching_should_throw_ReceiptCardAssertionFailedException_when_regex_matches_no_cards(string regex)
        {
        ...
        [Test]
        public void HasTaxMatching_should_throw_ReceiptCardAssertionFailedException_when_Tax_of_all_cards_is_null()
        {
            var cards = Enumerable.Range(1, 5).Select(_ => new ReceiptCard()).ToList();

            var sut = new ReceiptCardSetAssertions(cards);

            Action act = () => sut.HasTaxMatching(".*");

            act.ShouldThrow<ReceiptCardAssertionFailedException>();
        }

        [Test]
        public void HasTaxMatching_should_throw_ReceiptCardAssertionFailedException_when_trying_to_capture_groups_but_Tax_of_all_cards_is_null()
        {
            ...
            Action act = () => sut.HasTaxMatching(".*", "(.*)", out matches);

            act.ShouldThrow<ReceiptCardAssertionFailedException>();
        }

        [Test]
        public void HasTaxMatching_should_not_output_matches_when_regex_does_not_match_Tax_of_any_cards()
        {
        ...
        [Test]
        public void HasTaxMatching_should_not_output_matches_when_groupMatchingRegex_does_not_match_Tax_of_any_card()
        {
            IList<string> matches;
            ...
            sut.HasTaxMatching(".*", "(non matching)", out matches);

            matches.Should().BeNull();
        }

        [Test]
        public void HasTaxMatching_should_output_matches_when_groupMatchingRegex_matches_Tax_of_any_card()
        {
            IList<string> matches;
            ...
            sut.HasTaxMatching(someTax, $"({someTax})", out matches);

            matches.First().Should().Be(someTax);
        }

        [Test]
        public void HasTaxMatching_should_output_multiple_matches_when_groupMatchingRegex_matches_Tax_several_times_for_a_single_card()
        {
            IList<string> matches;
            ...
            sut.HasTaxMatching(someTax, $"({match1}) ({match2})", out matches);

            matches.Should().Contain(match1, match2);
        }

        [Test]
        public void HasTaxMatching_should_output_multiple_matches_when_groupMatchingRegex_matches_Tax_on_multiple_cards()
        {
            IList<string> matches;
            ...
            sut.HasTaxMatching(".*", @"(s[oa]me) (text)", out matches);
